...
w=ones(size(x));
switch scheme
    case {2,3,12,13, 32, 42, 52, 62, 82, 92, 102, 112, 122, 123, 132, 133}
        % Domain is aperiodic
...
switch scheme
    ...
    case {80,81,82,90,91,92,100,101,102,110,111,112,120,121,130,131}
        % Already done.
    case {122}
        % First copy the first 2 and last 2 rows from scheme 12:
        D(1,1)= -25/(12*dx);
        D(1,2)= 4/(dx);
        D(1,3)=-3/dx;
        D(1,4)=4/(3*dx);
    ...
    case {132}
        ...
        % Unlike scheme 12, we also need to set row end-2:
        %D=(-diag((1/6)*ones(N-2,1),2) + diag((4/3)*ones(N-1,1),1) - diag((4/3)*ones(N-1,1),-1) + diag((1/6)*ones(N-2,1),-2))/(2*dx);
        D(end-2,end-4)= 1/(12*dx);
        D(end-2,end-3)=-2/(3*dx);
        D(end-2,end-2)= 0;
        D(end-2,end-1)= 2/(3*dx);
        D(end-2,end-0)=-1/(12*dx);
    ...
    case {133}
        ...
        % Row end-2 is like the interior of scheme 90:
        D(end-2,:) = 0;
        D(end-2,end-3) = -1/(3*dx);
        D(end-2,end-2) = -1/(2*dx);
        D(end-2,end-1) =  1/(dx);
        D(end-2,end-0) = -1/(6*dx);
