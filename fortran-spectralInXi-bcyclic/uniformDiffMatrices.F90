...
subroutine uniformDiffMatrices(N, xMin, xMax, scheme, x, weights, ddx, d2dx2)
  ...
  real(rprec), intent(in) :: xMin, xMax
  real(rprec), intent(out), dimension(N) :: x, weights
  real(rprec), intent(out), dimension(N,N) :: ddx, d2dx2
  integer :: i
  real(rprec) :: dx, dx2
  real(rprec) :: h
  integer :: n1, n2
  real(rprec), allocatable :: topc(:), col1(:)
  real(rprec), parameter :: pi = 3.1415926535897932384626433d+0
