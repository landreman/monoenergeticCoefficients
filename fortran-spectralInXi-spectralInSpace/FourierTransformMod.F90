...
#include <petsc/finclude/petscsysdef.h>
  ...
  private
  PetscScalar, dimension(:,:), allocatable :: sinmtheta, cosmtheta, sinnzeta, cosnzeta
...
contains
  ...
  subroutine FourierTransform(original, vect)
    ...
    use variables, only: Ntheta, Nzeta, theta, zeta, xm, xn, NFourier, NPeriods
    
    implicit none
    
    PetscScalar, dimension(:,:), intent(in) :: original
    PetscScalar, dimension(:), intent(out) :: vect
  ...
  subroutine initFourierTrig
    ...
    use variables, only: Ntheta, Nzeta, mmax, nmax, theta, zeta, NPeriods
