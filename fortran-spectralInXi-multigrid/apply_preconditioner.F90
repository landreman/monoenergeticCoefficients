...
#include <petsc/finclude/petsckspdef.h>
  ...
  subroutine apply_preconditioner(outer_preconditioner, input_Vec, output_Vec, ierr)
    ...
    use petscksp
    use variables, only: masterProc, numProcs, myRank, inner_KSP, matrixSize, constraint_option, one, levels, &
         preconditioning_option
    use indices
...
!    logical :: verbose = .true.
    ...
    select case (preconditioning_option)
       ...
       case (2)
          ...
          if (constraint_option .ne. 1) stop "Error! preconditioning_option=1 requires constraint_option=1."
