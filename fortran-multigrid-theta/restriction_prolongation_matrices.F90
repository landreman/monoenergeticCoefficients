...
subroutine restriction_prolongation_matrices(fine_level)
  ...
  ! Allocate 2 nonzero entries per row:
  if (numProcs == 1) then
     call MatSeqAIJSetPreallocation(multigrid_prolongation_matrices(fine_level), 2, PETSC_NULL_INTEGER, ierr)
  else
     call MatMPIAIJSetPreallocation(multigrid_prolongation_matrices(fine_level), 2, PETSC_NULL_INTEGER, 2, PETSC_NULL_INTEGER, ierr)
  end if
